...
void main() {
     // 使用噪声实现烟雾、波纹效果
    // 1 第一种噪声   像马赛克 位置稍微有一点方正
    // float strength = step(0.5, noise(vUv * 100.0));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // 2 另一种噪声函数 位置大小不是很规则  有点水渍的感觉  step调高的话 有一点小石子的感觉
    // float strength = cnoise(vUv * 10.0 + uTime);
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
